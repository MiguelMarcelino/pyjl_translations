...
function compute_row(p::Tuple{Int64, Int64})::Tuple
    y, n = p
    pixels_ = pixels(y, n, abs)
    result = Vector{UInt8}([pixels_() for _ in (1:(n+7)÷8)])
...
if abspath(PROGRAM_FILE) == @__FILE__
    mandelbrot(parse(Int, append!([PROGRAM_FILE], ARGS)[2]))
end
