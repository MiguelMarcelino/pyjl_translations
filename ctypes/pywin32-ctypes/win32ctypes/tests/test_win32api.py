...
# import win32api
import win32ctypes.pywin32.win32api as win32api
...
class TestWin32API(unittest.TestCase):
    ...
    def load_library(self, module: Module, library=sys.executable, flags=0x2):
    ...
    @contextlib.contextmanager
    def resource_update(self, module: Module, library=sys.executable):
    ...
    def test_free_library(self):
        with self.load_library(win32api) as handle:
            # self.assertTrue(win32api.FreeLibrary(handle) is None) # The test returns true...
            self.assertTrue(win32api.FreeLibrary(handle) == 1)
